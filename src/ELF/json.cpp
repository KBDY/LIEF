  ...
  if (!reg_vals.empty()) {
    ...
    switch (pstatus.architecture()) {
      case ARCH::EM_386:
        {
          for (size_t i = 0; i < reg_vals.size(); ++i) {
            regs[to_string(static_cast<CorePrStatus::Registers::X86>(i))] = reg_vals[i];
          }
          break;
        }
      case ARCH::EM_X86_64:
        {
          for (size_t i = 0; i < reg_vals.size(); ++i) {
            regs[to_string(static_cast<CorePrStatus::Registers::X86_64>(i))] = reg_vals[i];
          }
          break;
        }
      case ARCH::EM_ARM:
        {
          for (size_t i = 0; i < reg_vals.size(); ++i) {
            regs[to_string(static_cast<CorePrStatus::Registers::ARM>(i))] = reg_vals[i];
          }
          break;
        }
      case ARCH::EM_AARCH64:
        {
          for (size_t i = 0; i < reg_vals.size(); ++i) {
            regs[to_string(static_cast<CorePrStatus::Registers::AARCH64>(i))] = reg_vals[i];
          }
          break;
        }
      default:
